...
__global__ void L1AccumLevel(float *AccumOut ,const uint32_t *SampleIn, int DataLen ,int width, GPU_L1_FreqPhase *Param)
{
    ...
    __shared__	uint32_t s_data_in[MAX_THREADS];
	...
	AccumOut[dout] = acum_sum/2048;///16;
}
...
//  IFダンプは外部で読み込んでおいて。
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
__host__ int GPU_L1AccumlevelCh1(float *Devout ,float *LevelOut ,int32_t width ,int32_t height ,size_t AccumLen ,GPU_L1_FreqPhase &CorrParam)
{
	int32_t ack,max;
	uint32_t *dev_in = (uint32_t*)((int8_t*)gIfData);	// 	(uint32_t*)gIfDump;	// 
	float *dev_out=Devout;
	int32_t *dev_iout=0;
	int32_t *dev_qout=0;

    GPU_L1_FreqPhase *dev_caparam = 0;
//--	GPU_L1_FreqPhase caparam;
    cudaError_t cudaStatus;
	GPU_L1_ThreadParam check;

//	caparam.Sv = 1;
//	caparam.fromCenterFreq(gSysFreqRes ,gL1CarrFreqRes ,500 ,height/2);
//	caparam.fromBaseCodePhase(gSysFreqRes ,0 ,0 ,0x80000000 ,0);
	...
	// パラメータ コピー
	cudaMemcpy((void*)dev_caparam ,(void*)&CorrParam ,sizeof(GPU_L1_FreqPhase) ,cudaMemcpyHostToDevice);
...
//		L1TrackingCh &tack = gL1TrackingCh[CorrParam.Sv];
		...
		// C/Aコードをコピー
		cudaStatus = cudaMemcpyToSymbol(devCaCode ,(const int8_t*)(&gL1CaCode[CorrParam.Sv][0]) ,1023 * sizeof(int8_t));
		if (cudaStatus != cudaSuccess) {
		...
		L1AccumLevel<<<grid, block>>>(dev_out ,dev_in ,AccumLen ,width ,dev_caparam);

		// 相関結果コピー
		cudaMemcpy((void*)LevelOut ,(void*)dev_out ,width*height*sizeof(float) ,cudaMemcpyDeviceToHost);
//		ack = maxChipPos(OutDataf ,2048*40);
		
//		tack.CodePhase		= ack % 2048;
//		tack.CarrDcoAcquire	= ack / 2048;
